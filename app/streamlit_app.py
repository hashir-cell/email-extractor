...
st.subheader("Login to Your Email Accounts")

providers = ["Gmail", "Outlook", "Yahoo"]  # Add more if needed
selected_providers = st.multiselect(
    "Select email providers to connect", providers
)
...
for provider in selected_providers:
    if provider == "Gmail":
        if st.button("Login with Gmail"):
            ...
            st.success("✅ Gmail logged in!")
    elif provider == "Outlook":
        if st.button("Login with Outlook"):
            st.session_state["outlook_logged_in"] = True
            st.success("✅ Outlook logged in!")
    elif provider == "Yahoo":
        if st.button("Login with Yahoo"):
            st.session_state["yahoo_logged_in"] = True
            st.success("✅ Yahoo logged in!")

connected_accounts = []
if st.session_state.get("gmail_logged_in"):
    connected_accounts.append("Gmail")
if st.session_state.get("outlook_logged_in"):
    connected_accounts.append("Outlook")
if st.session_state.get("yahoo_logged_in"):
    connected_accounts.append("Yahoo")

if connected_accounts:
    st.info(f"Connected Accounts: {', '.join(connected_accounts)}")
else:
    st.warning("No accounts connected yet.")
